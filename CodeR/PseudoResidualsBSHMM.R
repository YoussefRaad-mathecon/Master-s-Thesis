# BS–HMM: N-state Black–Scholes HMM
...
# ---- Parameter extraction (mu / sigma / mu_sigma) ----
# "mu"       : mu_j state-dependent, sigma shared
...
PseudoResiduals_HMM <- function(y, mod, N, pattern = c("mu","sigma","mu_sigma"), dt = 1/252) {
  ...
  u1 <- as.numeric(params$delta %*% pMat[1, ])
  ...
  for (i in 2:n) {
    ...
    ui <- as.numeric(a %*% weighted_Gamma %*% pMat[i, ])
    ui <- pmin(pmax(ui, eps), 1 - eps)
    Res[i] <- qnorm(ui)
  }
...
# ---- Pseudo-residuals (2–5 states) ----

# mu-only
pr_2_mu <- PseudoResiduals_HMM(x, models_BSHMM_2019$mod2_mu, N = 2, pattern = "mu", dt = dt)
pr_3_mu <- PseudoResiduals_HMM(x, models_BSHMM_2019$mod3_mu, N = 3, pattern = "mu", dt = dt)
...
safe_qq <- function(z, main, col_main = "#901a1E", col_ref = "steelblue", cex.main = 2.7, lwd = 3) {
  z <- clean_res(z)
  if (length(z) < 2) {
    plot.new()
  ...
  qqnorm(z, main = main, col = col_main, xlab = "", ylab = "", cex.main = cex.main)
  qqline(z, col = col_ref, lwd = lwd)
}

# ---- Small QQ plots (mu / sigma / mu&sigma) ----
par(oma = c(3,3,0,1), mar = c(3,3,2,3), mfrow = c(3,4))

## Row 1: mu-only
safe_qq(pr_2_mu$Res, "2-State HMM")
safe_qq(pr_3_mu$Res, "3-State HMM")
safe_qq(pr_4_mu$Res, "4-State HMM")
safe_qq(pr_5_mu$Res, "5-State HMM")

## Row 2: sigma-only
safe_qq(pr_2_sigma$Res, "2-State HMM")
safe_qq(pr_3_sigma$Res, "3-State HMM")
safe_qq(pr_4_sigma$Res, "4-State HMM")
safe_qq(pr_5_sigma$Res, "5-State HMM")

## Row 3: mu & sigma
safe_qq(pr_2_mu_sigma$Res, "2-State HMM")
safe_qq(pr_3_mu_sigma$Res, "3-State HMM")
safe_qq(pr_4_mu_sigma$Res, "4-State HMM")
safe_qq(pr_5_mu_sigma$Res, "5-State HMM")
...
# ---- Big QQ plots (mu / sigma / mu&sigma) ----
...
row_at <- rev(((1:length(Ns))        - 0.5) / length(Ns))
col_at <-      ((1:length(patterns)) - 0.5) / length(patterns)
