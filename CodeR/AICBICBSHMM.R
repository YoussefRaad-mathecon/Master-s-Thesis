...
summary_tbl %>%
  arrange(family, bic) %>%
  dplyr::select(family, N, k, negloglik, aic, bic, aic_rank, bic_rank, aic_weight, k_ok, conv_code) %>%
  print(n = Inf)
...
#   dplyr::select(Family = family, States = N, k, `-log L` = negloglik, AIC = aic, BIC = bic,
