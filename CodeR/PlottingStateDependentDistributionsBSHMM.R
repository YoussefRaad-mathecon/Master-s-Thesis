# BS–HMM: State-dependent densities (12 models)
...
# ---- Panel builder: histogram + state densities + mixture + fixed labels ----
plot_state_densities <- function(x, fit, N, family) {
  ...
  # Extra headroom depends on N (more states -> more vertical space)
  if (N <= 2) {
    y_max_factor <- 1.15
  } else if (N == 3) {
    y_max_factor <- 1.20
  ...
  # Vertical layout for labels: S1 at top, equally spaced
  top_frac <- 0.88
  gap_frac <- 0.048
