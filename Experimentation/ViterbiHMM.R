################################################################################################
# BS–HMM (4 states, μ & σ): Viterbi Decoding + State Path + Histogram with State Densities
################################################################################################
...
# For histogram section we also use a clean, NA-free vector:
...
################################################################################################
# 2) Viterbi decoding for 4-state BS–HMM (μ & σ)
################################################################################################
...
### Helper: unpack a BS–HMM parameter vector (same logic as your big grid script)
...
################################################################################################
# 3) Plots: BS–HMM state decoding + state path
################################################################################################

# define legend vertical position slightly below y_top
y_legend_top <- y_top - 0.01
y_step       <- 0.02
...
p_ret
################################################################################################
# 3b) BS–HMM state path + manual legend with state percentages (S1, S2, ...)
################################################################################################

# Legend x-position (moved further left than the return plot)
x_legend_state <- max(spx_states$Date) - 7 * 365  # tweak if you want it further left

# Build percentages for legend (3 significant digits, forced to sum to 100)
delta_pct       <- 100 * delta4_hat[1:N]
delta_pct_round <- signif(delta_pct, 3)
if (N > 1) {
  delta_pct_round[N] <- 100 - sum(delta_pct_round[1:(N - 1)])
}
...
# Legend vertical position (above state 4)
y_legend_top <- 4.6
y_step       <- 0.18
...
p_state_path <- spx_states %>%
  ...
  scale_color_manual(
    values = palette_states,
    guide  = "none"   # no ggplot legend; we use manual one
  ) +
  scale_x_date(
    date_breaks = "10 years",
    date_labels = "%Y"
  ...
  scale_y_continuous(
    breaks = 1:4,
    limits = c(1, 4.8)   # extra space at the top for legend
  ) +
  # S1
  annotate(
    "text",
...
################################################################################################
# 4) Histogram + State-dependent BS–HMM Return Densities + Mixture  (TWEAKED)
################################################################################################

title_size      <- 18
axis_title_size <- 16   # to match p_ret / p_state_path
axis_text_size  <- 12   # tick labels same size as other plots
annot_size      <- 5    # BIGGER legend text (like first plot)
line_width      <- 0.9
hist_alpha      <- 0.55

# Global x-range & grid for densities
...
plot_state_densities <- function(x, fit, N, family) {
  ...
  # ---------------- Legend / label geometry ----------------
  # Put labels clearly to the LEFT of the extreme right, roughly around 70% of the x-range
  x_label_text <- xrng[1] + 0.88 * xrange
  
  # Mask will start a bit left of the text and go all the way to the right
  ...
  # Histogram as density (full x-range)
  ...
  # Extra headroom depending on N, then cap at 100 as requested
  if (N <= 2) {
    y_max_factor <- 1.15
  } else if (N == 3) {
    y_max_factor <- 1.20
  ...
  y_max     <- min(y_max_raw, 100)   # CUT at density = 100
  
  # Mask only above histogram (so bars are fully drawn)
  y_mask_min <- min(hist_density_max * 1.02, y_max)  # ensure not above new max
  
  cols <- palette_states[seq_len(N)]
  
  # Vertical layout for labels
  top_frac <- 0.88      # S1 position as fraction of y_max
  gap_frac <- 0.048     # vertical gap between labels
  ...
  # Header just above S1
  header_y <- y_top_labels + 0.045 * y_max
  
  ggplot() +
    # Histogram over full x-range
    geom_histogram(
      data = data.frame(x = x),
    ...
    # State density lines (full x-range)
    geom_line(
      data = dens_df,
    ...
    # Mask band behind labels (top only)
    geom_rect(
      aes(
        xmin = x_mask_min,
        xmax = x_mask_max,
    ...
    # Fixed labels (larger text size now)
    geom_text(
      data = labels_df,
...
# Build 4-state (μ & σ) histogram panel
